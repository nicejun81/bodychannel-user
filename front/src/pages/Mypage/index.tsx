...
export const MyPage = () => {
  ...
  const [activeTab, setActiveTab] = useState<'profile' | 'my'>('profile')
  ...
  const header = (
    ...
    >
      <div className="flex">
        {([{ key: 'profile', label: '프로필' }, { key: 'my', label: '마이' }] as const).map((tab) => (
          <button
            key={tab.key}
    ...
    <PageLayout header={header} className="py-8">
      {activeTab === 'profile' ? (
        ...
        <div className="py-12 text-center">
          <p className="text-body text-ink-placeholder">준비 중입니다</p>
        </div>
      )}
    </PageLayout>
  )
